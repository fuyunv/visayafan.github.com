...
* git remote
  ...
  从远程仓库上用git clone下载下来的可以理解为有两个分支origin/master和master，其中origin为远程仓库名，前者分支是不受你支配的，你所在的是master分支。
  当push时，git会比较你的origion/master和远程仓库的是否一样，如果一样并且你有权限则可以push，则fast-forward。
  如果不一样则需要用 =git fetch= 下载下来，此时的远程仓库和你的仓库里的origin/master一致。merge后再push。
  * 查看当前repo有哪些远程repo，可以用 =git remote= ，可以加-v来查看详细信息（将显示出其远程仓库地址，例如cho45 git://github.com/cho45/grit.git）。
  ...
  * 取得remote repo的某个分支 =git fetch origin= 后 =git checkout -b branchName origin/branchName= ，后者是必须的，否则本地没有分支与origin的branchName对应。本地分支名不一定必须与remote分支名相同，如果相同的话，可以简化 =git checkout --track origin/branchName= 。
    也可以直接merge =git fetch origin= 后 =git merge origin/master= 。merge前可以查看diff =git diff master origin/master= 。
  * 删除服务器上remote分支 =git push origin :branchName= ，此命令中本地分支为空，表示删除remote分支。
* git fetch
  https://git-scm.com/book/zh/v1/Git-%E5%88%86%E6%94%AF-%E8%BF%9C%E7%A8%8B%E5%88%86%E6%94%AF
...
* git区间
  ...
  3. 02943a..a123ab 两个提交点之间的(左开右闭)
     02943a..默认是到最新提交点
* git处理pull request
  A fork B的repo后，便拥有了自己版本的repo版本，A可以对A的repo进行修改commit，不会对B的repo造成任何影响。
  A想要自己的代码合并到B中，必须发起pull request（github）或merge request（gitlab）。
  B看到A发起的request后，进行审核，决定是否合并到自己的分支上。
  B在命令行下操作：
  #+BEGIN_EXAMPLE
  // 得到A master分支上的代码
  git fetch http://202.120.4.52/visayafan/doc.git master
  // 创建名为A-branch的分支
  git checkout -b A-branch FETCH_HEAD
  测试A的代码
  // 切换到master
  git checkout master
  // 合并A-branch上的代码
...
* git origin master
  ...
  origin是个alias 表示remote repo。当clone时自动将remote repo命名为origin。也可以git remote add origin remote_repo_address。当然也可以起其它名。
  master表local repo的一个分支。
  origin/master 表示remote repo的master分支。
  HEAD表示当前工作空间的分支。 https://git-scm.com/book/zh/v1/Git-%E5%88%86%E6%94%AF-%E4%BD%95%E8%B0%93%E5%88%86%E6%94%AF
  #+BEGIN_EXAMPLE
  git push origin HEAD:master   // 表示将当前工作空间的分支 push 到 origin的master分支
...
* 搭建git服务器
  ...
  ssh-keygen -t rsa
  #+END_EXAMPLE
  服务器端(CentOS, IP为192.168.1.101)：
  #+BEGIN_EXAMPLE
  #安装git
  yum install perl openssh git
  #新建git用户，此用户专门用来处理git repo
  adduser --system --shell /bin/sh --create-home --home-dir /home/git git
  #+END_EXAMPLE
  将本地公钥保存到服务器上的/home/git/.ssh/authorized_keys中，并修改权限
  #+BEGIN_EXAMPLE
  chmod -R 700 ~/.ssh
  #+END_EXAMPLE
  测试本地是否可以无密码登陆，即ssh 192.168.1.101后提示类似Enter passphrase for key 'xxxxxx/.ssh/id_rsa'而非Enter user git password.
  ...
  如果要进行权限管理，需要配置gitolite或gitosis，参考http://www.uml.org.cn/pzgl/201206121.asp
* tricks
** git中永久删除一个文件
