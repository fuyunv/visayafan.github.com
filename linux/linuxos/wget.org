...
** 下载网页
   ...
   wget visayafan.com
   #+END_EXAMPLE
   原封不动地下载该网站的index.html.
   -k选项将链接转换以便适合本地查看：如果链接所指已下载到本地，则转换成指向本地的相对链接；否则转换成链接的完整URL。
   #+BEGIN_EXAMPLE
   wget -k visayafan.com
   #+END_EXAMPLE
** 下载网站
   #+BEGIN_EXAMPLE
   wget -m visayafan.com
   #+END_EXAMPLE
   把整个网站都下载下来
** 下载文件并重命名
   #+BEGIN_EXAMPLE
...
** 下载指定目录所有文件
   ...
   #+BEGIN_EXAMPLE
   wget -c -r -np -k -L -p ftp://ftp.cs.sjtu.edu.cn:990/lu-ht/DIP-Gonzalez/
   #+END_EXAMPLE
   1. -c continue，适合继续下载之前没下完的文件
   2. -r recursive
   3. -np no-parent 不下载父目录
   4. -k convert-link
   5. -L relative 只下载相对链接
   6. -p page-requisites 下载页面上所有东西（内嵌的图片、声音）
