...
#+title:Emacs Gnus
新闻组类似现在的BBS(或者说现在的BBS类似新闻组)，使用人数越来越少，但仍旧坚持使用的人中有很多大牛，可以从中学到很多东西，并且里面的技术氛围很深厚，说废话的人很少，这是我选择使用新闻组的原因。

要阅读新闻组中的内容首先需要一个服务器，此服务器中存在所有新闻组内容，客户端连接上后可以从上面下载所有新闻组的列表，然后从中订阅自己感兴趣的组。Gnus就是这样一个客户端，服务器你可以花钱买，服务肯定好，但对于我这样的穷人来说自然选择免费的。

新闻组服务器地址格式类似 news.example.xxx，国外的免费的比较快的是news.aioe.org和news.gmane.org.(更新2014年05月01日17:29: 最近发现了news.eternal-september.org，速度比aioe.org要快，但需要注册，登陆服务器需要用户名和密码才可以获得完整的新闻组列表，需要注意我的.gnus.el中(add-hook 'nntp-server-opened-hook 'nntp-send-authinfo)语句及上面的注释)
* 配置
  将下面配置保存到 ~/.emacs.d/gnus/.gnus.el中：
{{{srcbegin(200)}}}
...
{{{srcend}}}
  需要新建目录~/.emacs.d/gnus/news/cache，并在其中新建空文件active，否则报错。

  *注意* 不要随便修改gnus-select-method中的服务器地址，如若修改，则之前所有标记、下载的帖子全部作废。f需要使用 =M-x gnus-group-clear-data-on-native-groups= 删除之前的数据，将~/.emacs.d/gnus/news/cache目录删除。参考手册：https://www.gnu.org/software/emacs/manual/html_node/gnus/Changing-Servers.html
* 使用
  =M-x gnus= 便可进入Gnus。
  
  =gnus-group-enter-server-mode= 快捷键 =^= 可以查看所有服务器的连接状态， =opened= 正常， =denied= 表连接失败。

  =RET= 可进入相应的服务器。此时会下载此服务器上所有组列表，之后进入 =*Gnus Browse Server*= 缓冲区。可以使用 =list-matching-lines= 查找包含自己感兴趣的关键字的组，在组名上 =u= 即可订阅，订阅完后 =q= 回到 =server buffer= , =q= 回到 =group buffer= ，此时会看到刚才订阅的组出现了此buffer中，回车即可进入 =summary= 缓冲区，在帖子标题上空格即可在下面打开一个 =Article= 帖子弱冲区.
...
* 快捷键
  ...
  | C-u g                       | 查看此article的源始文件（比如想查看message-ID等信息时有用）                                        |
  | g                           | 重新显示当前article(可以加前缀指定字符集)，当C-u g查看源始文件后可用g恢复                          |
...
** 使用agent来下载帖子
   ...
   其实agent是个big cache, headers如果本地有会从本地读取，Gnus只下载一次headers但不会缓存articles，原因是太大（除非是自己标记下载的）.
*** agent工作流程：
    1. 假如现在本机没有连网， =J j= 将agent设置为unpluged状态，用户使用gnus阅读帖子(pluged状态且没有连网则guns是用不了的)。
...
*** 具体使用
    ...
    * 之后进入group buffer，可以选择查看帖子，也可以选择将所有组帖子下载下来或只下载一个组, 可以在agent->fetch菜单下可以看到。
      也可以下载某个帖子，先用 =#= 标记然后 =J s= 下载 *或* 用 =%= 标记（快捷键为 =@= ）然后 =J u= 下载。
      也可以下载指定几个组，此时需要使用分类(category).
    * category构成包括：名字，几个组，条件。
      所有组都在默认default分类中，此分类条件为short(只下载行数少于100行的article)。
      可以自已新建组： =J C= 进入category buffer，菜单categories下可以选择新建分类，为分类添加组，改变其条件。
...
** M-^取加指定message-id的article
   ...
   *另外*
   ...
   2. 如果知道message-id想要在gmane上阅读，链接为 http://mid.gmane.org/MESSAGE-ID ，比如上面的article对应的是 http://mid.gmane.org/8738w7c5fh.fsf@gmail.com
      =C-u g= 可以查看article的源始文件，里面有message-ID。
** C-c C-a邮件添加附件
...
** 删除draft中的article
   =B delete=
   菜单Article->Copy,move...(Backend)->delete article
** sticky article
   默认情况下在summary buffer里一个帖子的标题上回车将会打开这个帖子，而打开的这个帖子的buffer是重用了刚才的article buffer，即刚才的帖子被覆盖了。如果你想同时查看几个帖子就需要用到sticky article，在article上 =A S= ，之后再打开新帖子时会另开buffer来打开， =k= kill帖子， =gnus-kill-sticky-article-buffers= kill所有sticky帖子。
** 标记一个读过的帖子为未读
   =M c= 其中M表mark,c表clear
   或快捷键 =M-u= 
** 标记同主题的帖子已读
   =M k= 快捷键为k，但会跳转到下个未读的帖子
   =M K= 快捷键为 =C-k= ，不会跳转
